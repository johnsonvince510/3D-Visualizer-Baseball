...
function trackRoleLabel(role: string | null | undefined): string {
...
// Backwards-compatible alias for any lingering references inside previously parsed
// objects (e.g. when hot module reloading keeps older closures around).
function roleLabelFromRole(role: string | null | undefined): string {
  return trackRoleLabel(role);
}
...
async function onFiles(fs: FileList | null) {
  ...
  const normalizedTracks = playable.map((t, idx) => {
    ...
    const roleLabel = trackRoleLabel(role);
